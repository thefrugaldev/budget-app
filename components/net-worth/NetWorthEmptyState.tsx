...
/**
 * First-run state for the Net Worth page (#109 chunk 6, story 17): explains what
 * an account is and the four types the page tracks, so the page is
 * self-explanatory on day one. The actual "Add account" trigger is wired in
 * chunk 7 (it's an edit affordance, gated by `useCanEdit`) — this read-only
 * chunk sets the scene and the copy points at it.
 */
const TYPES = [
...
export function NetWorthEmptyState() {
  return (
    <div className="mx-auto max-w-xl rounded-2xl bg-card p-8 text-center ring-1 ring-border">
      ...
      <p className="mt-6 text-sm text-muted-foreground">Add your first account to get started.</p>
    </div>
  );
}
