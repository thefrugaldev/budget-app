...
2. **Investment value is quantities × live market prices.** Prices come from a `PriceProvider` interface with a Mongo-backed quote cache and a per-holding manual price override. The provider is a swap, not a commitment — and it was swapped: the first implementation (Finnhub free tier, `FINNHUB_API_KEY`) returns `c: 0` for mutual funds, which a real portfolio is full of (401k / index / target-date funds), so the current provider is **Tiingo's end-of-day daily endpoint** (`TIINGO_API_KEY`), which prices stocks, ETFs, **and** mutual-fund NAVs from one API (#143). EOD is the right granularity — funds price once a day at NAV, and the app is monthly-cadence with a 12h cache, so nothing downstream (`resolveQuotes`, the cache, `accountValue`, the override, the UI) changed. Tiingo also backs the add-holding ticker `/search` autocomplete (#144): the original plan kept search on Finnhub to spare Tiingo's quota, but Finnhub's free `/search` returns **nothing** for mutual funds (verified against `VTSAX`/`VTIAX`/`FXAIX`), so it couldn't find most of a real portfolio's holdings — Tiingo's `utilities/search` matches what the quote path can price, so Finnhub was **removed entirely** and Tiingo is the sole market-data provider. Search is debounced + min-length + session-cached, a trickle against the 50 req/hour free tier. The manual override remains the guaranteed fallback for anything even Tiingo can't quote.
