...
- **Google is the sole *authentication* method — but email address must be an enabled *identifier*.** Only the Google SSO connection can sign a user in; **email/password and email-code *sign-in* stay disabled** (Password especially — enabling it would let an invited user bypass Google, unacceptable for a finance app). Email address, however, has to be **enabled as a sign-up identifier**, because Clerk's invitation and account-linking flows are email-keyed: with it off you cannot create an invitation or provision an invitee by email. Google OAuth supplies the verified email that links to the pre-created/invited record. *(An earlier revision of this note claimed email-as-identifier was disabled — that was wrong and it silently blocked the whole invite flow; corrected 2026-07-07 after live testing.)*
