...
**4. Signature element — "Momentum" on Pulse.** `components/budget/pulse/GrowthColumns.tsx` renders a server-side static-SVG stack of trailing monthly columns — spending on the bottom, saving stacked above — climbing toward a dashed "plan" line (the month's total caps + goals). It expresses the product's job in one glance: progress against Targets, over time. It is the one bold thing; everything around it stays quiet. Chart geometry is shared (`lib/charts/scale.ts`) rather than bespoke — see [[chart_primitives_refactor]] and ADR-driven convention "Charts compose from shared primitives" in `AGENTS.md`.
