...
/**
 * Pure cadence/paycheck arithmetic. Deliberately free of any `@/lib/budget`
 * dependency so that `lib/budget` (the lower primitive layer) can use these from
 * `computeIncomeForRange` without a `budget → income → budget` import cycle — the
 * budget-dependent income helpers live in `./index`, which re-exports this file
 * so `@/lib/income` consumers see one flat public API.
 */
 ...
 * persisted (story 4), and by `computeIncomeForRange` to weight each expected
 * paycheck (story 9). `monthly` cadence is a pass-through, mirroring the forward
 * helper.
 */
